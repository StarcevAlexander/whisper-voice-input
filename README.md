...
| Действие | Результат |
|---|---|
| Скажите **"центральная"** | Начинается запись |
| Скажите **"марш"** | Запись останавливается, текст распознаётся через Whisper и вставляется в активное поле |
...
> Защита от повторного запуска: при старте скрипт проверяет, что другой экземпляр не запущен. Если процесс уже работает — новый экземпляр завершится автоматически.
...
| `WAKE_PHRASE` | `центральная` | Фраза для начала записи (режим `voice`) |
| `STOP_PHRASE` | `марш` | Фраза для остановки записи (режим `voice`) |
